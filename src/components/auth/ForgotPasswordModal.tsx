...
export function ForgotPasswordModal({ isOpen, onClose }: ForgotPasswordModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resetError) {
        if (resetError.message.includes('rate limit') || resetError.message.includes('For security purposes')) {
      ...
      >
        ...
        {step === 'sent' && (
          <div className="text-center py-2">
            ...
            <p className="font-semibold text-slate-800 dark:text-slate-100 mb-5 break-all text-sm">
              {email}
            </p>
