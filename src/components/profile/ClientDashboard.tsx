  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-emerald-900/40 bg-slate-900/60 p-4 shadow-sm">
          <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-slate-300">
            <Heart className="h-4 w-4 text-emerald-400" />
            <span>{t.profile.favorites}</span>
          </div>
          <p className="text-3xl font-bold text-emerald-400">{totalFavorites}</p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
            {Object.entries(goals).map(([key, value]) => (
              <div
                key={key}
                className="rounded-xl bg-slate-900/60 p-3 text-center shadow-sm"
              >
                <div className="text-xl font-bold text-emerald-400 sm:text-2xl">
                  {value}
                </div>
                <div className="text-xs capitalize text-slate-400 sm:text-sm">
                  {key}
                </div>
              </div>
            ))}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {favoriteRecipesList.slice(0, 3).map((recipe) => (
              <div
                key={recipe.id}
                className="flex items-center gap-4 rounded-xl bg-slate-900/60 p-4 shadow-sm"
              >
                <img
                  src={recipe.image}
                  alt={recipe.title}
